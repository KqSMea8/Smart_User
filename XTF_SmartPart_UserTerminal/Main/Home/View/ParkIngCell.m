...
@interface ParkIngCell()<carMessageDelegate>
{
...
- (void)_createView {
    _bgScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, 131)];
    _bgScrollView.showsVerticalScrollIndicator = NO;
    _bgScrollView.showsHorizontalScrollIndicator = YES;
    _bgScrollView.pagingEnabled = YES;
    [self.contentView addSubview:_bgScrollView];
    
    _bindBtn = [[UIButton alloc] initWithFrame:CGRectMake(15*wScale, 0, KScreenWidth-30*wScale, 50)];
    _bindBtn.centerY = self.contentView.centerY;
    _bindBtn.layer.cornerRadius = 6;
    _bindBtn.backgroundColor = [UIColor colorWithHexString:@"#1B82D1"];
    [_bindBtn setTitle:@"+ 绑定车辆" forState:UIControlStateNormal];
    ...
    [_bgScrollView addSubview:_bindBtn];
    _bindBtn.hidden = YES;
}

-(void)setIsHidden:(NSString *)isHidden{
    _isHidden = isHidden;
...
- (void)setCarLists:(NSArray *)carLists {
    _carLists = carLists;
    
    _bgScrollView.contentSize = CGSizeMake(KScreenWidth * carLists.count, 0);
    
    if (carLists == nil || carLists.count == 0) {
    ...
    for (int i=0; i<carLists.count; i++) {
        CarRecordModel *carRecordModel = carLists[i];
        CarMessageView *carMessageView = [[CarMessageView alloc] initWithFrame:CGRectMake(i*KScreenWidth, 0, KScreenWidth, self.height)];
        carMessageView.delegate = self;
        carMessageView.tag = 100+i;
        carMessageView.currentTime = _currentTime;
        carMessageView.carRecordModel = carRecordModel;
        [_bgScrollView addSubview:carMessageView];
        
//        UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(i*KScreenWidth, 0, KScreenWidth, self.height)];
...
//            [bgView addSubview:cancleBookBtn];
//
...
//            [bgView addSubview:recordBgView];
//
...
//            carNoBgImgView.image = [UIImage imageNamed:@"car_no_bg_blue"];
//            [bgView addSubview:carNoBgImgView];
//
...
//            carNolabel.textAlignment = NSTextAlignmentCenter;
//            [bgView addSubview:carNolabel];
//
//            // 停车场
//            UILabel *parkNamelabel = [[UILabel alloc] initWithFrame:CGRectMake(carNoBgImgView.right + 20, 20, KScreenWidth - carNoBgImgView.right - 32, 17)];
...
//            [bgView addSubview:parkNamelabel];
//
//            // 进场时间
...
//            [bgView addSubview:timeLongLabel];
//            if(![carRecordModel.traceBegin isKindOfClass:[NSNull class]]&&carRecordModel.traceBegin != nil){
...
#pragma mark carMessageDelegate
