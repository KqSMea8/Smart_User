...
#import "YQDropDownView.h"
#import "YQDropView.h"

#ifdef YQLogFlag
#define YQLog(FORMAT,...) fprintf(stderr,"**********HGBErrorLog-satrt***********\n{\n文件名称:%s;\n方法:%s;\n行数:%d;\n提示:%s\n}\n**********HGBErrorLog-end***********\n",[[[NSString stringWithUTF8String:__FILE__] lastPathComponent] UTF8String],[[NSString stringWithUTF8String:__func__] UTF8String], __LINE__, [[NSString stringWithFormat:FORMAT, ##__VA_ARGS__] UTF8String]);
#else
#define YQLog(...);
#endif
...
@interface YQDropDownView()<UITableViewDelegate, UITableViewDataSource>{
    
}

@property (nonatomic, strong) UIWindow * window;
/** 主题容器view */
@property (nonatomic, strong) YQDropView * topicContainerView;
/** 装载tableView的背景容器 */
@property (nonatomic, strong) YQDropView * tableViewContainerView;
...
- (void)setUpUI{
    //主题容器view
    self.topicContainerView = [[YQDropView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame))];
    ...
    //装载tableView的背景容器
    self.tableViewContainerView = [[YQDropView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.topicContainerView.frame) + 3, CGRectGetWidth(self.frame), _numberOfRowsToDisplay * _heightForRow)];
    ...
    self.tableView.layer.borderColor = [UIColor colorWithHexString:@"#1B82D1"].CGColor;
 ...
 */
- (void)resetUI{
    self.tableViewContainerView.frame = CGRectMake(0, CGRectGetMaxY(self.topicContainerView.frame) + 3, CGRectGetWidth(self.frame), _numberOfRowsToDisplay * _heightForRow);
    self.tableViewContainerView.layer.shadowPath = [UIBezierPath bezierPathWithRect:_tableViewContainerView.bounds].CGPath;
    ...
    if (_pattern == kDropDownPatternCustom){
        if ([self.delegate respondsToSelector:@selector(viewForTopicInDropDown:)]) {
            ...
            YQLog(@"请通过- (UIView *)viewForTopicInDropDown:(ZFDropDown *)dropDown协议方法返回一个自定义topic view");
        }
    }
}
...
- (void)show{
    ...
    if (self.topicContainerView.isSelected) {
        ...
        [UIView animateKeyframesWithDuration:0.1f delay:0.f options:UIViewAnimationOptionAllowUserInteraction &UIViewAnimationOptionCurveEaseIn animations:^{
            ...
            self.frame=CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width,self.topicContainerView.frame.size.height);
//            +_heightForRow*self.itemArray.count
...
- (void)hideOtherDropDown{
    for (UIView * subview in self.superview.subviews) {
        if ([subview isKindOfClass:[YQDropDownView class]]) {
            YQDropDownView * dropDown = (YQDropDownView *)subview;
            if (subview != self) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_pattern == kDropDownPatternDefault) {
        ...
        if ([self.itemArray[indexPath.row] isKindOfClass:[NSString class]]) {
            ...
            YQLog(@"当前dropDown样式为kDropDownPatternDefault, 请替换为kDropDownPatternCustom");
...
- (void)setDelegate:(id<YQDropDownDelegate>)delegate{
    _delegate = delegate;
...
- (void)setBorderStyle:(kDropDownTopicBorderStyle)borderStyle{
    ...
    if (_borderStyle == kDropDownTopicBorderStyleSingleLine) {
        ...
        self.topicContainerView.layer.borderWidth = 1.0f;
    }
}
