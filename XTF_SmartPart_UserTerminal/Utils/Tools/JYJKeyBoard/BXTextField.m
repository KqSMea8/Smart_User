...
//        kbHeight = kbEndFrame.size.height - 44;
    ...
    // 按钮的位置计算
    CGFloat doneButtonX = 6;
    CGFloat doneButtonW = 0;
    CGFloat doneButtonH = 0;
    // 为了适配不同屏幕
    if ([UIScreen mainScreen].bounds.size.width == 320) {
        doneButtonW = ([UIScreen mainScreen].bounds.size.width - doneButtonX*4) / 3;
        doneButtonH = (kbHeight - 4*4) / 4;
    ...
    UIButton *doneButton = [[UIButton alloc] initWithFrame:CGRectMake(doneButtonX, doneButtonY - 4, doneButtonW, doneButtonH)];
    doneButton.titleLabel.font = [UIFont systemFontOfSize:37];
...
//    UIImage *btBgImage = [UIImage imageNamed:@"keyboard02"];
//    btBgImage = [btBgImage resizableImageWithCapInsets:UIEdgeInsetsMake(20, 20, 20, 20) resizingMode:UIImageResizingModeStretch];
//    [doneButton setBackgroundImage:btBgImage forState:UIControlStateNormal];
