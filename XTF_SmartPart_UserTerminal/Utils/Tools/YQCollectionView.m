...
#pragma mark - UICollectionView (YQDragCellCollectionViewRect)

@implementation UICollectionView (YQDragCellCollectionView_BMRect)

- (CGRect)YQDragCellCollectionView_rectForSection:(NSInteger)section {
    NSInteger sectionNum = [self.dataSource collectionView:self numberOfItemsInSection:section];
    if (sectionNum <= 0) {
        return CGRectZero;
    }
    CGRect firstRect = [self YQDragCellCollectionView_rectForRowAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
    CGRect lastRect = [self YQDragCellCollectionView_rectForRowAtIndexPath:[NSIndexPath indexPathForItem:sectionNum-1 inSection:section]];
...
- (CGRect)YQDragCellCollectionView_rectForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [self layoutAttributesForItemAtIndexPath:indexPath].frame;
}

@end

typedef NS_ENUM(NSUInteger, YQDragCellCollectionViewScrollDirection) {
    YQDragCellCollectionViewScrollDirectionNone = 0,
    YQDragCellCollectionViewScrollDirectionLeft,
    YQDragCellCollectionViewScrollDirectionRight,
    YQDragCellCollectionViewScrollDirectionUp,
    YQDragCellCollectionViewScrollDirectionDown,
};

@interface YQCollectionView ()
...
@implementation YQCollectionView
...
// 处理临界点问题
- (YQDragCellCollectionViewScrollDirection)_setScrollDirection {
    if (self.bounds.size.height + self.contentOffset.y - _snapedView.center.y < _snapedView.bounds.size.height / 2 && self.bounds.size.height + self.contentOffset.y < self.contentSize.height) {
        ...
        return YQDragCellCollectionViewScrollDirectionRight;
    } else if (_snapedView.center.x - self.contentOffset.x < _snapedView.bounds.size.width / 2 && self.contentOffset.x > 0) {
        return YQDragCellCollectionViewScrollDirectionLeft;
    }
    return YQDragCellCollectionViewScrollDirectionNone;
}
...
- (void)_edgeScroll {
    YQDragCellCollectionViewScrollDirection scrollDirection = [self _setScrollDirection];
    switch (scrollDirection) {
        case YQDragCellCollectionViewScrollDirectionLeft:{
            //这里的动画必须设为NO
            ...
            break;
        case YQDragCellCollectionViewScrollDirectionRight:{
            [self setContentOffset:CGPointMake(self.contentOffset.x + 4, self.contentOffset.y) animated:NO];
            _snapedView.center = CGPointMake(_snapedView.center.x + 4, _snapedView.center.y);
            _lastPoint.x += 4;
        }
            break;
        case YQDragCellCollectionViewScrollDirectionUp:{
            ...
            break;
        case YQDragCellCollectionViewScrollDirectionDown:{
            [self setContentOffset:CGPointMake(self.contentOffset.x, self.contentOffset.y + 4) animated:NO];
            _snapedView.center = CGPointMake(_snapedView.center.x, _snapedView.center.y + 4);
            _lastPoint.y += 4;
        }
    ...
    if (scrollDirection == YQDragCellCollectionViewScrollDirectionNone) {
        return;
    }
            ...
            if (longGesture.isEnabled) {
                if (self.delegate
                    ...
                    for (NSInteger i = 0; i < sec; i++) {
                        if (CGRectContainsPoint([self YQDragCellCollectionView_rectForSection:i], point)) {
                            section = i;
                            break;
                        }
                    }
                ...
                [UIView animateWithDuration:0.25 animations:^{
                    ...
                    cell.hidden = NO;
                    self.userInteractionEnabled = YES;
                    if (self.delegate && [self.delegate respondsToSelector:@selector(dragCellCollectionViewDidEndDrag:indexPath:)]) {
                        [self.delegate dragCellCollectionViewDidEndDrag:self indexPath:index];
                    }
                }];
            }
            // 关闭定时器
...
- (void)dragMoveItemToIndexPath:(NSIndexPath *)newIndexPath {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.userInteractionEnabled = YES;
        if (self.delegate && [self.delegate respondsToSelector:@selector(dragCellCollectionViewDidEndDrag:indexPath:)]) {
            [self.delegate dragCellCollectionViewDidEndDrag:self indexPath:newIndexPath];
        }
    }];
    // 关闭定时器
    _oldIndexPath = nil;
